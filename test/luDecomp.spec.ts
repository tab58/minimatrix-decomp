...
import { LUSolver } from '../src/index';

const EPS = 1e-14;
describe('Decomposition Methods', (): void => {
  describe('LU Decomposition', (): void => {
    it('should solve an Ax=b problem for a 2x2 matrix', (): void => {
      const M = new Matrix2();
      M.setElements(1, 2, 3, -5);
      const { P, A } = LUSolver.decompose(M);
      const b = new Vector2(4, 1);
      const x = LUSolver.solve(A, P, b);
